...
| | |
|---|---|
| **Receives from** | 05 App Integrator (first run) or 07 Fixer (subsequent runs) |
...
2. On first clean PASS: write to `{TEAM_KNOWLEDGE_ROOT}/{TECH_STACK_KEY}/build_gotchas.md` any config pattern that proved error-prone during this project. Update Projects Contributed in `{TEAM_KNOWLEDGE_ROOT}/team_knowledge.md`.
...
```
cd {PROJECT_ROOT} && ./gradlew assembleDebug --stacktrace 2>&1
```

### Step 2 — tests
```
cd {PROJECT_ROOT} && ./gradlew test 2>&1
```
...
- Category: MissingImport | UnresolvedReference | TypeMismatch | MissingResource | GradleConfig | Other
- Context: <2–3 lines of surrounding code>

## Notable Warnings
(Only warnings that indicate real problems — deprecated API in a public surface, unchecked casts, etc.)

## Summary
- Total errors: N
- Modules affected: [list]
- Recommended fix priority: [Gradle config first → core → features → app]
```
