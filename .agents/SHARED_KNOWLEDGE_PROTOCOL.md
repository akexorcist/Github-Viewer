...
| Core / Feature Developer | `effective_patterns.md`, `lessons_learned.md` |
| App Integrator | `effective_patterns.md`, `build_gotchas.md` |
...
- **Resource Registry** — every new resource file entry
...
After writing, update the Projects Contributed table in `{TEAM_KNOWLEDGE_ROOT}/team_knowledge.md`.
...
```
`ClassName` → `fully.qualified.package.ClassName` (module/src/main/kotlin/.../ClassName.kt)
```
...
**Resource Registry** entry format:
```
`@type/name` in `module` (path/to/file.xml)
```
